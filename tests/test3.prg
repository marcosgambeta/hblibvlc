...
// Compile with:
// hbmk2 test3 -gtgui ../hblibvlc.hbc

#include <winapi_windows.ch>

#define IDM_FILE_PLAY 1
#define IDM_FILE_PAUSE 2
#define IDM_FILE_SNAPSHOT 3
...
PROCEDURE Main()
   ...
   s_player := libvlc_media_player_new(vlc_instance)
   ...
   libvlc_media_player_set_media(s_player, media)

   // Set the player to render in the window
   libvlc_media_player_set_hwnd(s_player, hwnd)

   waShowWindow(hwnd, SW_SHOW)
   waUpdateWindow(hwnd)

   // Play the video
   libvlc_media_player_play(s_player)
   ...
   // clean up
   libvlc_media_player_stop(s_player)
   libvlc_media_release(media)
   libvlc_media_player_release(s_player)
...
FUNCTION WindowProc(hwnd, uMsg, wParam, lParam)
   ...
   CASE WM_COMMAND
      ...
      CASE IDM_FILE_PLAY
         ...
         EXIT
      CASE IDM_FILE_PAUSE
         libvlc_media_player_pause(s_player)
         EXIT
      CASE IDM_FILE_SNAPSHOT
         IF libvlc_video_take_snapshot(s_player, 0, ;
               "snapshot" + StrTran(DToS(Date()), "/", "") + StrTran(Time(), ":", "") + ".png", ;
               0, 0) == 0
         ELSE
            waMessageBox(hwnd, "Snapshot failed", "Info", MB_OK)
         ENDIF
         EXIT
      CASE IDM_FILE_QUIT
...
STATIC FUNCTION CreateMenus(hwnd)
   ...
   waAppendMenu(hMenu, MF_STRING, IDM_FILE_PLAY, "&Play")
   waAppendMenu(hMenu, MF_STRING, IDM_FILE_PAUSE, "P&ause/Continue")
   waAppendMenu(hMenu, MF_STRING, IDM_FILE_SNAPSHOT, "&Snapshot")
