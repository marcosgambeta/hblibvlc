...
{
  libvlc_dialog_set_context((libvlc_dialog_id *)hb_parptr(1), (void *)hb_parptr(2));
}

// LIBVLC_API void * libvlc_dialog_get_context(libvlc_dialog_id *p_id)
HB_FUNC(LIBVLC_DIALOG_GET_CONTEXT)
{
  hb_retptr((void *)libvlc_dialog_get_context((libvlc_dialog_id *)hb_parptr(1)));
}
...
{
  hb_retni(libvlc_dialog_post_action((libvlc_dialog_id *)hb_parptr(1), hb_parni(2)));
}

// LIBVLC_API int libvlc_dialog_dismiss(libvlc_dialog_id *p_id)
HB_FUNC(LIBVLC_DIALOG_DISMISS)
{
  hb_retni(libvlc_dialog_dismiss((libvlc_dialog_id *)hb_parptr(1)));
}
